use actix_web::web;
use sqlx::mysql::MySqlQueryResult;
use crate::AppState;
use crate::models::user::{RegisterUser, UpdateUser, User};

pub async fn get_all(app_state: web::Data<AppState>) -> Result<Vec<User>, sqlx::Error> {
    let result = sqlx::query_as!(User, "SELECT * FROM user")
        .fetch_all(&app_state.sql_client)
        .await;

    result
}

pub async fn create(app_state: web::Data<AppState>, user: web::Json<RegisterUser>) -> Result<MySqlQueryResult, sqlx::Error> {
    let pool = app_state.sql_client.clone();
    let result = sqlx::query!(
        "INSERT INTO user (username, email, password) VALUES (?, ?, ?)",
        user.username,
        user.email,
        user.password
    )
    .execute(&pool)
    .await;
    
    result
}

pub async fn update(app_state: web::Data<AppState>, id: u64, user: web::Json<UpdateUser>) -> Result<UpdateUser, sqlx::Error> {
    let pool = app_state.sql_client.clone();
    let result = sqlx::query!(
        "UPDATE user SET username = ?, email = ?, password = ? WHERE id = ?",
        user.username,
        user.email,
        user.password,
        id
    )
    .execute(&pool)
    .await;

    result
        .map(|_| user.into_inner())
        .map_err(|e| e.into())
}